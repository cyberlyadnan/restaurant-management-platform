...
export function CheckoutPanel({
  ...
  const remainingDue = round2(
    Math.max(0, totalDue - giftCardApplied - alreadyPaid),
  );
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-card">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 overscroll-contain">
        ...
        <div className="rounded-2xl border border-border/70 bg-gradient-to-br from-muted/40 to-muted/10 p-3.5 shadow-2xs">
          <div className="flex items-center justify-between">
            ...
            {order.status === "PARTIALLY_PAID" || alreadyPaid > 0 ? (
              <span className="rounded-md bg-amber-500/15 px-2 py-0.5 text-[10px] font-bold text-amber-600 dark:text-amber-400 uppercase tracking-wider">
                Partially Paid
              </span>
            ) : (
          ...
          <div className="mt-1 flex items-baseline justify-between">
            ...
            <span className="text-xs font-semibold text-muted-foreground">
              {alreadyPaid > 0 ? "Settlement In Progress" : "Bill Pending"}
            </span>
          </div>
          ...
          {alreadyPaid > 0 && (
            <div className="mt-2.5 pt-2 border-t border-border/50 flex items-center justify-between text-xs">
              <span className="text-muted-foreground font-medium">
                Collected ({order.payments?.length ?? 1} payment{(order.payments?.length ?? 1) === 1 ? "" : "s"}):
              </span>
        ...
        {/* Split Bill Calculator & Multi-Tender Trigger */}
        <div className="flex items-center justify-between rounded-xl border border-border/70 bg-muted/20 p-2.5">
          <div className="flex items-center gap-2">
            <Button
              type="button"
              variant="outline"
              ...
              className="h-8 gap-1.5 px-3 text-xs font-bold border-primary/30 text-primary hover:bg-primary/10 transition-colors shadow-2xs"
            >
            ...
            {splitCount > 1 && (
              <span className="text-xs font-bold text-muted-foreground tabular-nums">
                ≈ {formatCurrency(round2(remainingDue / splitCount))} / p
              </span>
            )}
          </div>
        ...
        <div className="space-y-1 text-xs">
          ...
          <div className="border-t border-border/60 pt-1.5 flex justify-between items-baseline">
            <span className="text-sm font-bold text-foreground">
              {alreadyPaid > 0 ? "Remaining Due" : "Total Due"}
            </span>
      ...
      <SplitBillDialog
        ...
        onFullyPaid={(completedOrder) => {
          setCompleted(completedOrder);
